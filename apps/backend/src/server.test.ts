...
describe('서버 시작 시 시드 데이터 초기화', () => {
    ...
    it('노드 ID로 개별 노드를 조회할 수 있어야 한다', () => {
        const node = store.getNodeById('R-01');
        expect(node).toBeDefined();
        expect(node!.type).toBe('Resource');
        expect(node!.name).toBe('Global Lithium Reserves');
    });
    ...
    it('특정 노드의 인바운드 엣지를 조회할 수 있어야 한다', () => {
        // M-01은 R-01에서 공급받는 엣지가 있어야 한다
        const inboundEdges = store.getInboundEdges('M-01');
        expect(inboundEdges.length).toBeGreaterThanOrEqual(1);
        expect(inboundEdges.some(e => e.sourceNodeId === 'R-01')).toBe(true);
    });

    it('DataStore 인터페이스를 통해 모든 노드 타입이 포함되어야 한다', () => {
        const nodes = store.getNodes();
