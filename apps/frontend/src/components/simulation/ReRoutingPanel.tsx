...
export const ReRoutingPanel: React.FC<ReRoutingPanelProps> = ({
    ...
    if (isRerouteLoading) {
        ...
        return (
            <Card className="border border-border bg-muted/40 shadow-sm shrink-0 h-[280px] max-h-[280px] flex flex-col justify-center items-center text-center p-4">
    ...
    return (
        <Card className="border border-border bg-muted/40 shadow-sm shrink-0 h-[280px] max-h-[280px] flex flex-col overflow-hidden text-xs text-foreground">
            {/* 1. 헤더 (시나리오 프리셋 카드와 동일한 CardHeader) */}
            <CardHeader className="p-3 pb-1 shrink-0 border-b border-border/40">
                <CardTitle className="text-xs font-bold text-primary flex items-center gap-1.5 truncate">
                    ...
                    <span className="truncate">대체 공급망 시나리오 ({mainResult.targetNodeName})</span>
                </CardTitle>
            </CardHeader>

            <CardContent className="p-3 pt-2.5 flex-1 min-h-0 flex flex-col overflow-y-auto space-y-2.5">
                {/* 2. 세그먼트 컨트롤 탭 (박스 중첩 피로도 방지) */}
                {plans.length > 0 && (
